...
<div class="table-responsive">
	<table class="table display responsive product-overview mb-30" id="shopItemsTable">
        ...
        <th><i class="fa fa-money"></i> <?php echo direction("Selling Price","سعر البيع") ?></th>
		</tr>
		</thead>
		...
		<?php 
        $JoinData = array(
            "select" => ["t.enTitle as productTitle", "t.id as productId", "t1.enTitle as categoryTitle","t2.enTitle as brandTitle","t3.quantity","t3.sku","CAST(ROUND(t3.cost, 3) AS CHAR) AS costPrice","CAST(ROUND(t3.price, 3) AS CHAR) AS sellingPrice"],
		...
		if( $products = selectJoinDBNew("products", $JoinData,[$_GET["id"]],"t.id != 0 AND t.shopId = ? ORDER BY t.id DESC") ){
			for( $i = 0; $i < sizeof($products); $i++ ){
                ...
                <td class="text-right"><strong><?php echo number_format($products[$i]["sellingPrice"], 3) ?></strong></td>
				</tr>
				<?php
			}
		}
...
$(document).ready(function() {
    ...
    var table = $('#shopItemsTable').DataTable({
        ...
        columnDefs: [
            ...
            { targets: [4, 6, 7], className: 'text-right' }, // SKU and Cost Price columns
