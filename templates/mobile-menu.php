					...
					<div id="accordion1" class="my-list">
						<div class="card">
							<div class="card-header mobile-drop-cust-close">
								<a class="card-link collapsed" data-toggle="collapse" href="#Mcollapse0"><h6 class="product-category-mobile" type="0"><img src="https://i.imgur.com/H2EYo9m.png" width="18px" height="18px" class="ml-3 mr-3"><?php echo " " ;echo direction("All","الكل"); ?></h6></a>
							</div>
							<div id="Mcollapse0" class="collapse" data-parent="#accordion1"></div>
						</div>
						<?php 
						if( $category = selectDB("categories","`status` = '0' AND `hidden` = '1' AND `storeId` = '{$storeID}' ORDER BY `rank` ASC") ){
							for($i=0; $i<sizeof($category); $i++){
							?>
							<div class="card">
								<div class="card-header mobile-drop-cust-close">
									<a class="card-link collapsed" data-toggle="collapse" href="#Mcollapse<?php echo $category[$i]["id"] ?>"><h6 class="product-category-mobile <?php if ($category[$i]["glow"] == 1 ){echo 'glow" style="font-weight:700';} ?>" type="<?php echo $category[$i]["id"] ?>"><img src="<?php echo encryptImage("logos/{$category[$i]["imageurl"]}") ?>" style="18px" height="18px" class="ml-3 mr-3"><?php echo " " ;echo direction($category[$i]["enTitle"],$category[$i]["arTitle"]); ?></h6></a>
								</div>
